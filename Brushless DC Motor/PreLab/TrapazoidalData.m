...
fluxC=zeros(n+1,1);
torque=zeros(n+1,1);
%make currents a trapazoidal waveform and make them out of phase from each other by 120 degrees
%T=180degrees, A = 1, theta = 0, 120, 240
off=45/2;%need to apply an offset so that first current starts at the same phase as induced voltages
IA = 2*(asin(sin(2*pi*(degrees+off)/180))+acos(cos(2*pi*(degrees+off)/180)))/pi-1;
IB = 2*(asin(sin(2*pi*(degrees+off+120)/180))+acos(cos(2*pi*(degrees+off+120)/180)))/pi-1;
IC = 2*(asin(sin(2*pi*(degrees+off+240)/180))+acos(cos(2*pi*(degrees+off+240)/180)))/pi-1;
...
torque=[['Torque'];num2cell(torque)];

Trapazoidal=[degrees,IA,IB,IC,fluxA,fluxB,fluxC,torque];
xlswrite('TrapazoidalData',Trapazoidal)